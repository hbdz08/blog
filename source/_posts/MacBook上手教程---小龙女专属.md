...
### 桌面
不同于windows桌面，macOS桌面比较简洁，安装后的软件，不会在桌面放置快捷方式。macOS上一般都是放一些常用的文件或文件夹。

#### 整理桌面
桌面犹如生活中我们自己的房间，需要时常整理收纳，要不然一段时间过后，就会被无数文档占满屏幕。
![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260732609768.jpg)
整理的第一步是将文档的位置规整摆放。通常手动移动桌面上的文件时，都是随意放在某个位置。在桌面上点击鼠标右键，在菜单中选择「整理」，所有文件就会被对齐放置。但此时文件之间可能还存在空隙。在菜单中找到整理方式选项，选择自己希望的方式，就会看到文件从屏幕右上方开始依次排列。
![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260733411197.jpg)


除了通过「整理」功能，还可以使用「排序」功能直接到达同样的效果。不过需要注意的是，如果选择了排序，所有文件都不能再被自由地移动，而是必须依次排列。

![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260733989230.jpg)

不管是整理还是排序，都只能将文件排得整齐。如果文件的数目很多，依然会占满屏幕。此时，可以使用「叠放」功能。还是在右键菜单中，选择使用叠放，再调整叠放分组方式，就会看到文件被收纳到不同的组中，单击图标就可展开显示组内所有文件。这一功能可谓是懒人收纳的大杀器。

最后，无论选择什么整理方式，在右键菜单中点击查看显示选项，都可以根据自己的偏好。调整文件图标的大小和间距。
#### 更改桌面壁纸
在桌面右键菜单中选择更改桌面背景…，就可以更改桌面壁纸。

![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260735429454.jpg)

桌面壁纸的来源分为三种：

1. 自带壁纸：系统默认桌面图片文件夹中的图片
2. 照片：照片应用中的图片
3. 文件夹中的图片：用户创建的文件夹中的图片

其中自带壁纸又分为动态、浅色和深色桌面以及普通的桌面图片。

其实，最直接的办法是在你喜爱的图片上直接点击右键，选择设定桌面图片即可。

### 访达
访达，类似于Windows中的‘我的电脑’，在macOS中是没有硬盘分区的，也就是说，没有所谓的C盘、D盘。在访达可以看到你电脑中所有的文件及应用。

![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/20210712105516.png)

可以通过对文件打标签的方式进行高效管理文件
 
![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260594116788.jpg)
![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260595313362.jpg)

另外，你想要在访达新建文件，比如ppt文件，是不支持右键新建的，只能新建文件夹。
![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260605574407.jpg)

你要新建ptt文件，需要在ptt软件中新建后保存。这点和Windows有所不同。
![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260606766654.jpg)
...
![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260615676175.jpg)

从左至右，程序坞中的三个区域被两条分隔线分开：

1. 常用 App：固定在程序坞中显示，可以自定义调节；
2. 已启动的应用：和 Windows 的任务栏一样，显示所有前台和后台正在运行的应用；
3. 特殊应用：这里会显示下载文件夹、最小化的应用和废纸篓。

另外，如果有其他 iOS 设备，并打开了「接力」功能，当在 iOS 设备上开启 Mac 上也有应用时，就会出现应用的「接力」图标。

![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260695878750.jpg)

#### 自定义程序坞
和菜单栏一样，点击苹果菜单 > 系统偏好设置 > 程序坞与菜单栏，来设置程序坞的可选项。

![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260697722878.jpg)
 
 在菜单中，还可以调节程序坞上应用图标的大小，以及鼠标滑过时，图标放大的比例；程序坞的位置，以及是否隐藏等等选项，这里不一一解释，大家可以自行尝试不同选项的效果。
 
 图标下面有个黑色的点，代表该应用正在运行中。

想调整应用在程序坞中的位置，按住鼠标左键拖动图标即可。将已启动的应用拖拽到常用 App 区域中，就相当于把图标设成常用 App。将常用 App （在应用没运行的情况下）拖出程序坞，直到看到「移除」后放开鼠标左键，就将它从常用 App 中移除了。

也可以右键，选项 > 在程序坞保留 ，设置为常用App。
![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260699964980.jpg)

放心，应用此时并没有被删除或卸载掉，只是移除出了程序坞而已。

### 菜单栏

我们刚刚已经说了屏幕的下半部分，这回我们再来说说屏幕的上半部分。也就是菜单栏。
...
### 菜单栏中的操作
点击苹果图标的菜单，点击关于本机，可以看到设备的基本信息，包括硬件配置和软件版本。
![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260738836903.jpg)


点击存储空间，可以看到剩余的存储空间，点击管理，可以进入系统自带的空间管理器。
![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260742035199.jpg)
![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260744037596.jpg)

通过检查文件，可以查看大文件的占用空间的大小
![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260744500722.jpg)

控制中心，和iPhone操作一样，不需要过多解释。
![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260748294957.jpg)

通知中心，点击右上角可以调出。
![](https://file-1256865446.cos.ap-guangzhou.myqcloud.com/img/2021/07/12/16260749526080.jpg)
